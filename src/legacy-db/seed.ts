/**
 * EpochBridge — Legacy Database Seeder
 *
 * Simulates a legacy AS/400-style relational database with:
 *   - CUSTMAST  (Customer Master file)
 *   - ORDERHDR  (Order Header file)
 *   - ORDERDTL  (Order Detail / Line Items)
 *   - INVNTRY   (Inventory file)
 *
 * Column naming follows COBOL/RPG 8-char conventions.
 */

import { getLegacyDb, closeLegacyDb } from './connection';
import { logger } from '../utils/logger';

function seed(): void {
  const db = getLegacyDb();

  logger.info('[Seed] Dropping and recreating legacy tables...');
  ...
  db.exec(`
    ...
    -- Customer Master (AS/400 CUSTMAST PF)
    CREATE TABLE IF NOT EXISTS CUSTMAST (
      ...
      CUST_ZIP    TEXT,
      CUST_PHONE  TEXT,
      CUST_CRDT   REAL    DEFAULT 0.00,   -- Credit limit (PACKED DECIMAL)
      CUST_STAT   TEXT    DEFAULT 'A',    -- A=Active, I=Inactive, S=Suspended
      CUST_CRTD   TEXT,                   -- Created date (YYYYMMDD)
      CUST_UPDT   TEXT                    -- Last updated
    );

    -- Inventory Master (INVNTRY PF)
    CREATE TABLE IF NOT EXISTS INVNTRY (
      ITEM_ID     TEXT    PRIMARY KEY,
      ITEM_DESC   TEXT    NOT NULL,
      ITEM_UOM    TEXT    DEFAULT 'EA',   -- Unit of measure
    ...
    CREATE TABLE IF NOT EXISTS ORDERHDR (
      ...
      ORDR_SHIP   TEXT,
      ORDR_STAT   TEXT    DEFAULT 'O',   -- O=Open, C=Closed, X=Cancelled
      ORDR_TOTAL  REAL    DEFAULT 0.00,
      ORDR_NOTES  TEXT
    );

    -- Order Detail / Line Items (ORDERDTL PF)
    CREATE TABLE IF NOT EXISTS ORDERDTL (
      ...
      DTAIL_UPRC  REAL    DEFAULT 0.00,  -- Unit price at time of order
      DTAIL_EXT   REAL    DEFAULT 0.00   -- Extended price (QTY * UPRC)
    );
  `);

  logger.info('[Seed] Tables created. Inserting seed data...');
  ...
  logger.info('[Seed] ✅ Legacy database seeded successfully!');
  logger.info('[Seed]    Tables: CUSTMAST (5), INVNTRY (8), ORDERHDR (4), ORDERDTL (7)');
